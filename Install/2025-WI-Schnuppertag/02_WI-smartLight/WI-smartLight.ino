...
/***********************************************************************************
 * ⚠️ AB HIER NICHT MEHR BEARBEITEN! ⚠️
 ***********************************************************************************/
...
void setup() {
  ...
  // Hostname und AP SSID mit Benutzernamen initialisieren
  hostname = "smartlight-" + getDeinName();
  ap_ssid = "smartLight-Config-" + getDeinName();
...
void clearConfig() {
  ...
  wifi_ssid = "";
  wifi_password = "";
  mqtt_server = "ae6eb0d482a24683b08611859374ec88.s1.eu.hivemq.cloud";
  mqtt_port = 8883;
  mqtt_user = "testHTL";
  mqtt_password = "iOT12XX23by";
  mqtt_topic = "schnuppern";
  mqtt_enabled = false;
...
void setupMQTT() {
  if (mqtt_server.length() > 0) {
    ...
    mqttClient.setKeepAlive(15); // 15 Sekunden Keep-Alive
    mqttClient.setBufferSize(512); // 512 Byte Buffer
...
void reconnectMQTT() {
  ...
  if (millis() - lastAttempt < 20000) return; // 20 Sekunden zwischen Versuchen
  lastAttempt = millis();
  ...
  if (currentTime - lastButtonChange > 50) { // 50ms Entprellzeit
    if (currentButtonState && !buttonPressed) {
      ...
      if (pressDuration > 8000) {
        // Lange Taste gedrückt (>8 Sekunden) = Reset
